...
唯一的的 [全局对象](http://www.ecma-international.org/ecma-262/7.0/#global-object) 是在控制流进入任何 [可执行上下文](http://www.ecma-international.org/ecma-262/7.0/#sec-execution-contexts) 之前创建的。
...
[全局对象](http://www.ecma-international.org/ecma-262/7.0/#global-object) 的内置 [[Prototype]] 内部属性值是依赖于实现的。

除了本规范定义的属性之外，[全局对象](http://www.ecma-international.org/ecma-262/7.0/#global-object) 还可以拥有额外的宿主定义的属性。[全局对象](http://www.ecma-international.org/ecma-262/7.0/#global-object) 可以包含其值是该全局对象自身的属性；例如在HTML文档对象模型中，全局对象的window属性是全局对象本身。
 
 
...
 
“Infinity” 的值是 +∞ (参照 [6.1.6](#sec-ecmascript-language-types-number-type))。 Infinity拥有特性 { [[Writable]]: false, [[Enumerable]]: false, [[Configurable]]: false }。

#### 18.1.2 NaN [#](http://www.ecma-international.org/ecma-262/7.0/#sec-value-properties-of-the-global-object-nan)

“NaN”的值是 NaN(参照 [6.1.6](#sec-ecmascript-language-types-number-type))。NaN拥有特性 { [[Writable]]: false, [[Enumerable]]: false, [[Configurable]]: false }。
### 18.1.3 undefined [#](http://www.ecma-international.org/ecma-262/7.0/#sec-undefined)
“undefined” 的值是 undefined(参照 [6.1.6](#sec-ecmascript-language-types-number-type))。 undefined拥有特性 { [[Writable]]: false, [[Enumerable]]: false, [[Configurable]]: false }。
...
“eval” 函数是 %eval% 内在对象。当使用一个参数 x 调用 eval 函数时，将执行以下步骤：

1. 让 evalRealm 是[活动函数对象](http://www.ecma-international.org/ecma-262/7.0/#active-function-object)的 [[Realm]] 内部槽的值。
2. 让 strictCaller 为false。
3. 让 directEval 为false。
...
isFinite函数是 %isFinite% 内在对象。当使用一个参数 number 调用 isFinite 函数时，将执行以下步骤：

 1. 让 num = ToNumber(number)。
 2. 如果 num 为 NaN, +∞, or -∞, 返回 false。
 3. 否则，返回 true。

### 18.2.3 isNaN (number) [#](http://www.ecma-international.org/ecma-262/7.0/#sec-isnan-number)

isNaN函数是 %isNaN% 内在对象。当使用一个参数 number 调用 isNaN 函数时，将执行以下步骤：

  1. 让 num 为 ToNumber(number)。
  2. 如果 num 为 NaN, 返回 false。
  3. 否则，返回 true。

  注意：ECMAScript 代码测试值 X 是否为 NaN 的可靠方法是 X !== X 形式的表达式。当且仅当 X 是 NaN 时，结果为真。
...
'parseFloat' 函数是 %parseFloat% 内在对象，当使用一个参数 string 调用 parseFloat 函数时，将执行以下步骤：
 ...
 2. 让 trimmedString 为 inputString 的子字符串， 它由 inputString 的非 StrWhiteSpaceChar 字符的最左边字符和它右边跟着的所有字符组成。( 换句话说 , 删掉前面的空白。) 如果 inputString 不包含任何这样的字符 , 则令 trimmedString 为空字符串。
 3. 如果 trimmedString 或 trimmedString 的任何前缀都不满足 StrDecimalLiteral ( 参照 [7.1.3.1](#sec-tonumber-applied-to-the-string-type)) 的语法 , 返回 NaN。
 4. 让 numberString 为满足 StrDecimalLiteral 语法的 trimmedString 的最长前缀，可能是 numberString 自身。
 ...
 7. 返回 mathFloat 的 Number 值。
    ...
    注意：parseFloat可以仅将字符串的前导部分解释为数值; 它忽略所有不能解释为数值字面量记法的一部分的字符，并且任何这样的代码单元被忽略都没有指示。


 ### 18.2.5 parseInt (string, radix) [#](http://www.ecma-international.org/ecma-262/7.0/#sec-parseint-string-radix)
 parseInt 函数根据指定的 radix 来解释 string 参数 产生一个整数值。string开头的空白会被忽略。如果 radix 是 undefined 或 0，则假定 radix 为10， 除非数字是以字符对 0x 或 0X 开头的，这时假定 radix 是 16。如果 radix 是 16，数字开头的字符对 0x 或 0X 是可选的。

 'parseInt' 函数是 %parseInt% 内在对象。 当 parseInt 函数被调用时，将执行以下步骤：

 1. 让 inputString 为 ToString（string）。
 2. 让 S 为一个新创建的子字符串，它由 inputString 的第一个非 StrWhiteSpaceChar 字符和它后面跟着的所有字符组成。( 换句话说 , 删掉前面的空白。) 如果 inputString 不包含任何这样的字符 , 则令 S 为空字符串。
 3. 让 sign 为 1。
 4. 如果 S 不是空并且 S 的第一个字符是 0x002D (减号 -), 则令 sign 为−1。
 5. 如果 S 不是空并且 S 的第一个字符是 0x002B（加号 +）或 0x002D (减号 -), 则删除 S 的第一个字符。
 6. 让 R = ToInt32(radix) 。
 7. 让 stripPrefix 为 true。
 8. 如果 R ≠ 0, 则
 ...
 12. 如果 Z 是空 , 返回 NaN.
 13. 让 mathInt 为 Z 的 radix-R 进制表示的数学值，用字母 A-Z 和 a-z 来表示 10 到 35 之间的值。( 但是 , 如果 R 是 10 并且 Z 包含多余 20 位的值 , 可以替换 20 位后的每个数字为 0, 这是实现可选的功能 ; 如果 R 不是 2, 4, 8, 10, 16, 32, 则 mathInt 可以是 Z 的 radix-R 进制表示的依赖于实现的近似值。)
 14. 如果 mathInt = 0，则
    a. 如果 sign = -1， return -0
    b. 返回 +0
 15. 让 number 为 mathInt 的数值。
 16. 返回 sign × number。

    注意：parseInt 可以仅将字符串的前导部分解释为整数值; 它忽略所有不能解释为整数值记法的一部分的字符，并且任何这样的代码单元被忽略都没有指示。
...
#### 18.2.6.1 URI 语法与语义 [#](http://www.ecma-international.org/ecma-262/7.0/#sec-uri-syntax-and-semantics)
 一个 URI 是由组件分隔符分割的组件序列组成。其一般形式是：
    <p>Scheme : First / Second ; Third ? Fourth</p>
...
##### 18.2.6.1.1 运行时语义: 编码 ( <var>string</var>, <var>unescapedSet</var> ) [#](http://www.ecma-international.org/ecma-262/7.0/#sec-encode)
...
4.  重复
    ...
    4.  否则 , C 不在 unescapedSet 里
        ...
        3.  否则,
            ...
            5.  令 V = [UTF16Decode](http://www.ecma-international.org/ecma-262/7.0/#sec-utf16decode)(<var>C</var>, <var>kChar</var>)。
...
##### 18.2.6.1.2 运行时语义: 解码 ( <var>string</var>, <var>reservedSet</var> ) [#](http://www.ecma-international.org/ecma-262/7.0/#sec-decode)
...
在 UTF-8 中，用 1 到 6 个字节的序列来编码字符。序列中唯一的高阶位字节设置为 0，其余的 7 位才用于编码字符值。在一个 n 个字节的序列中，n > 1，初始字节有 n 个设置为 1 的高阶位，其后的位设置为 0。这个字节的其他位包含是用来编码字符的比特。后面跟着的其字节都包含设定为 1 的高阶位，并且都跟着设定为 0 的位，剩下的 6 位都用作编码字符。[Table 44](http://www.ecma-international.org/ecma-262/7.0/#table-43) 指定了 ECMAScript 字符可能的 UTF-8 编码。
<figure>
<figcaption>Table 44 (Informative): UTF-8 编码</figcaption>
 <table>
   <tbody>
    <tr>
     <th>字符编码值</th>
...
#### 18.2.6.2 decodeURI (<var>encodedURI</var>) [#](http://www.ecma-international.org/ecma-262/7.0/#sec-decodeuri-encodeduri)
...
1.  令 <var>uriString</var> 为 [ToString](http://www.ecma-international.org/ecma-262/7.0/#sec-tostring)(<var>encodedURI</var>)。
2.  令 <var>reservedURISet</var> 为包含 [uriReserved](http://www.ecma-international.org/ecma-262/7.0/#prod-uriReserved) 每个有效字符加上 "#" 的实例的字符串。
3.  返回 [Decode](http://www.ecma-international.org/ecma-262/7.0/#sec-decode)(<var>uriString</var>, <var>reservedURISet</var>)。

注意：#" 字符不会从转义序列中解码，即使它不是 URI 保留字符。



#### 18.2.6.3 decodeURIComponent (<var>encodedURIComponent</var>) [#](http://www.ecma-international.org/ecma-262/7.0/#sec-decodeuricomponent-encodeduricomponent)
...
“decodeURIComponent” 函数是 %decodeURIComponent% 内在对象。当以一个参数 encodedURIComponent 调用 decodeURIComponent 函数，将执行以下步骤：
 ...
 1. 令 componentString 为 [ToString](http://www.ecma-international.org/ecma-262/7.0/#sec-tostring)(<var>encodedURIComponent</var>)。
 2. 令 reservedURIComponentSet 为一个空字符串。
 3. 返回调用 [Decode](http://www.ecma-international.org/ecma-262/7.0/#sec-decode)(<var>componentString</var>, <var>reservedURIComponentSet</var>) 的结果。

#### 18.2.6.4 encodeURI (<var>uri</var>) [#](http://www.ecma-international.org/ecma-262/7.0/#sec-encodeuri-uri)
...
encodeURI 函数计算 UTF-16 编码（6.1.4）URI的新版本，其中某些代码点的每个实例由代表代码点的UTF-8编码的一个，两个，三个或四个转义序列替换。

encodeURI 函数是内置 %encodeURI% 对象，当以一个参数 uri 调用 encodeURI 函数，将执行以下步骤：

1. 令 uriString 为 [ToString](http://www.ecma-international.org/ecma-262/7.0/#sec-tostring)(<var>uri</var>)。
...
#### 18.2.6.5 encodeURIComponent (<var>uriComponent</var>) [#](http://www.ecma-international.org/ecma-262/7.0/#sec-encodeuricomponent-uricomponent)

encodeURIComponent 函数计算 UTF-16 编码（6.1.4）URI的新版本，其中某些代码点的每个实例由代表代码点的UTF-8编码的一个，两个，三个或四个转义序列替换。

encodeURIComponent 是内置的 %encodeURIComponent% 对象，当以一个参数 uriComponent 调用 encodeURIComponent 函数，将执行以下步骤：

1. 令 componentString 为 [ToString](http://www.ecma-international.org/ecma-262/7.0/#sec-tostring)(<var>uriComponent</var>)。
...
3. 返回调用 [Encode](http://www.ecma-international.org/ecma-262/7.0/#sec-encode)(componentString, unescapedURIComponentSet) 的结果。
...
参照 [22.1.1](http://www.ecma-international.org/ecma-262/7.0/#sec-array-constructor)
...
参照 [19.2.1](http://www.ecma-international.org/ecma-262/7.0/#sec-function-constructor)
...
参照 [19.5.5.2](http://www.ecma-international.org/ecma-262/7.0/#sec-native-error-types-used-in-this-standard-rangeerror)
...
参照 [19.5.5.3](http://www.ecma-international.org/ecma-262/7.0/#sec-native-error-types-used-in-this-standard-referenceerror)

### 18.3.21 RegExp ( . . . )[#](http://www.ecma-international.org/ecma-262/7.0/#sec-constructor-properties-of-the-global-object-regexp)

参照 [21.2.3](http://www.ecma-international.org/ecma-262/7.0/#sec-regexp-constructor)
### 18.3.22 Set ( . . . )[#](http://www.ecma-international.org/ecma-262/7.0/#sec-set)

参照 [23.2.1](http://www.ecma-international.org/ecma-262/7.0/#sec-set-constructor)

### 18.3.23 String ( . . . )[#](http://www.ecma-international.org/ecma-262/7.0/#sec-constructor-properties-of-the-global-object-string)

参照 [21.1.1](http://www.ecma-international.org/ecma-262/7.0/#sec-string-constructor)

### 18.3.24 Symbol ( . . . )[#](http://www.ecma-international.org/ecma-262/7.0/#sec-constructor-properties-of-the-global-object-symbol)

参照 [19.4.1](http://www.ecma-international.org/ecma-262/7.0/#sec-symbol-constructor)
...
参照 [19.5.5.4](http://www.ecma-international.org/ecma-262/7.0/#sec-native-error-types-used-in-this-standard-syntaxerror)

### 18.3.26 TypeError ( . . . )[#](http://www.ecma-international.org/ecma-262/7.0/#sec-constructor-properties-of-the-global-object-typeerror)

参照 [19.5.5.5](http://www.ecma-international.org/ecma-262/7.0/#sec-native-error-types-used-in-this-standard-typeerror)
...
参照 [19.5.5.6](http://www.ecma-international.org/ecma-262/7.0/#sec-native-error-types-used-in-this-standard-urierror)

### 18.3.32 WeakMap ( . . . )[#](http://www.ecma-international.org/ecma-262/7.0/#sec-constructor-properties-of-the-global-object-weakmap)

参照 [23.3.1](http://www.ecma-international.org/ecma-262/7.0/#sec-weakmap-constructor)

### 18.3.33 WeakSet ( . . . )[#](http://www.ecma-international.org/ecma-262/7.0/#sec-constructor-properties-of-the-global-object-weakset)

参照 [23.4](http://www.ecma-international.org/ecma-262/7.0/#sec-weakset-objects)
 
 
 
 
...
参照 [24.3](http://www.ecma-international.org/ecma-262/7.0/#sec-json-object)

### 18.4.2 Math [#](http://www.ecma-international.org/ecma-262/7.0/#sec-math)

参照 [20.2](http://www.ecma-international.org/ecma-262/7.0/#sec-math-object)

### 18.4.3 Reflect [#](http://www.ecma-international.org/ecma-262/7.0/#sec-reflect)

参照 [26.1](http://www.ecma-international.org/ecma-262/7.0/#sec-reflect-object) 
 
