...
3.  令 script 是ECMAScript代码，它是解析x的结果，解释为目标 symbol [Script](http://www.ecma-international.org/ecma-262/7.0/#prod-Script) 的 如[6.1.4](http://www.ecma-international.org/ecma-262/7.0/#sec-ecmascript-language-types-string-type) 中描述的UTF-16编码的Unicode文本。如果解析失败, 抛出 SyntaxError 异常。 如果检测到任何早期错误，则根据错误的类型 (但另见 [16](http://www.ecma-international.org/ecma-262/7.0/#sec-error-handling-and-language-extensions))抛出一个SyntaxError或ReferenceError异常。 解析和 [early error](http://www.ecma-international.org/ecma-262/7.0/#early-error) 检测可以以实现相关的方式交织。
...
8.  令 ctx 为 [running execution context](http://www.ecma-international.org/ecma-262/7.0/#running-execution-context)。 如果 direct 为 true, ctx 将为执行 [direct eval](http://www.ecma-international.org/ecma-262/7.0/#sec-function-calls-runtime-semantics-evaluation) 的 [execution context](http://www.ecma-international.org/ecma-262/7.0/#sec-execution-contexts)。 如果 direct 为 false, ctx 将为调用 **eval** 函数的 [execution context](http://www.ecma-international.org/ecma-262/7.0/#sec-execution-contexts)。
...
13.  令 evalCxt 为新的 ECMAScript 代码[execution context](http://www.ecma-international.org/ecma-262/7.0/#sec-execution-contexts)。
...
19.  把 evalCxt 推送到 [execution context stack](http://www.ecma-international.org/ecma-262/7.0/#execution-context-stack); evalCxt 现在是 [running execution context](http://www.ecma-international.org/ecma-262/7.0/#running-execution-context).
...
23.  挂起 evalCxt 并将其从[execution context stack](http://www.ecma-international.org/ecma-262/7.0/#execution-context-stack)中删除。
24.  将现在位于 [execution context stack](http://www.ecma-international.org/ecma-262/7.0/#execution-context-stack) 顶部的上下文恢复为 [running execution context](http://www.ecma-international.org/ecma-262/7.0/#running-execution-context)。
25.  返回 [Completion](http://www.ecma-international.org/ecma-262/7.0/#sec-completion-record-specification-type)(result)。

注意
如果调用上下文正在评估形式参数初始化器，或者调用上下文或eval代码是严格代码，eval代码不能在调用上下文的调用上下文的变量环境中实例化变量或函数绑定。相反，这种绑定在一个只有eval代码可访问的新VariableEnvironment中实例化。let，const或类声明引入的绑定总是在新的LexicalEnvironment中实例化。
