...
1. 令 evalRealm 是[活动函数对象](http://www.ecma-international.org/ecma-262/7.0/#active-function-object)的 [[Realm]] 内部属性的值。
...
3.  Let script be the ECMAScript code that is the result of parsing x, interpreted as UTF-16 encoded Unicode text as described in [6.1.4](http://www.ecma-international.org/ecma-262/7.0/#sec-ecmascript-language-types-string-type), for the goal symbol [Script](http://www.ecma-international.org/ecma-262/7.0/#prod-Script). 如果解析失败, 抛出 SyntaxError 异常。 If any early errors are detected, throw a SyntaxError or a ReferenceError exception, depending on the type of the error (but see also clause [16](http://www.ecma-international.org/ecma-262/7.0/#sec-error-handling-and-language-extensions)). Parsing and [early error](http://www.ecma-international.org/ecma-262/7.0/#early-error) detection may be interweaved in an implementation dependent manner.
...
7.  否则, 令 strictEval 为 script 的 IsStrict。
8.  令 ctx 为 [running execution context](http://www.ecma-international.org/ecma-262/7.0/#running-execution-context)。 如果 direct 为 true, ctx 将为 [execution context](http://www.ecma-international.org/ecma-262/7.0/#sec-execution-contexts) that performed the [direct eval](http://www.ecma-international.org/ecma-262/7.0/#sec-function-calls-runtime-semantics-evaluation). 如果 direct 为 false, ctx will be the [execution context](http://www.ecma-international.org/ecma-262/7.0/#sec-execution-contexts) for the invocation of the `eval` function.
9.  如果 direct 为 true, 那么
    1.  令 lexEnv 为 [NewDeclarativeEnvironment](http://www.ecma-international.org/ecma-262/7.0/#sec-newdeclarativeenvironment)(ctx 的 LexicalEnvironment)。
    2.  令 varEnv 为 ctx 的 VariableEnvironment.
10.  否则,
    1.  令 lexEnv 为 [NewDeclarativeEnvironment](http://www.ecma-international.org/ecma-262/7.0/#sec-newdeclarativeenvironment)(evalRealm.[[GlobalEnv]]).
    2.  令 varEnv 为 evalRealm.[[GlobalEnv]].
11.  如果 strictEval 为 true, 令 varEnv 为 lexEnv。
12.  如果 ctx is not already suspended, suspend ctx.
13.  令 evalCxt 为 new ECMAScript code [execution context](http://www.ecma-international.org/ecma-262/7.0/#sec-execution-contexts).
14.  Set the evalCxt's Function to null.
15.  Set the evalCxt's [Realm](http://www.ecma-international.org/ecma-262/7.0/#realm) to evalRealm.
16.  Set the evalCxt's ScriptOrModule to ctx's ScriptOrModule.
17.  Set the evalCxt's VariableEnvironment to varEnv.
18.  设置 lexEnv 为 evalCxt 的 LexicalEnvironment。
19.  把 evalCxt push 到 [execution context stack](http://www.ecma-international.org/ecma-262/7.0/#execution-context-stack) 之上; evalCxt 现在是 [running execution context](http://www.ecma-international.org/ecma-262/7.0/#running-execution-context).
...
21.  如果 result.[[Type]] 是 normal, 那么
    1.  令 result evaluating body 的结果。
22.  如果 result.[[Type]] 是 normal 并且 result.[[Value]] 是 empty, 那么
    1.  令 result 为 [NormalCompletion](http://www.ecma-international.org/ecma-262/7.0/#sec-normalcompletion)(undefined).
23.  Suspend evalCxt and remove it from the [execution context stack](http://www.ecma-international.org/ecma-262/7.0/#execution-context-stack).
24.  Resume the context that is now on the top of the [execution context stack](http://www.ecma-international.org/ecma-262/7.0/#execution-context-stack) as the [running execution context](http://www.ecma-international.org/ecma-262/7.0/#running-execution-context).
25.  返回 [Completion](http://www.ecma-international.org/ecma-262/7.0/#sec-completion-record-specification-type)(result)。

注意 The eval code cannot instantiate variable or function bindings in the variable environment of the calling context that invoked the eval if the calling context is evaluating formal parameter initializers or if either the code of the calling context or the eval code is strict code. Instead such bindings are instantiated in a new VariableEnvironment that is only accessible to the eval code. Bindings introduced by `let`, `const`, or `class` declarations are always instantiated in a new LexicalEnvironment.
...
5.  如果 strict 为 false, 那么
    ...
    4.  如果thisLex 与 varEnv 不同就重复,
        ...
        2.  如果 thisEnvRec 不是一个对象 object [Environment Record](http://www.ecma-international.org/ecma-262/7.0/#sec-environment-records), 那么
            1.  注意: with语句的环境不能包含任何词法声明，因此不需要检查 var/let 提升冲突。
            2.  对 varNames 中的每个 name 执行，
                1.  如果 thisEnvRec.HasBinding(name) 为 true, 那么
                    1.  抛出 SyntaxError 异常。
                2.  注意: [direct eval](http://www.ecma-international.org/ecma-262/7.0/#sec-function-calls-runtime-semantics-evaluation) 不会将var声明提升到类似命名的词法声明。
...
9.  注意: 附录 [B.3.3.3](http://www.ecma-international.org/ecma-262/7.0/#sec-web-compat-evaldeclarationinstantiation) 关于这一点添加了额外的步骤。
...
**parseFloa**t 函数根据将 string 参数的内容解释为十进制字面量来产生一个数值。
